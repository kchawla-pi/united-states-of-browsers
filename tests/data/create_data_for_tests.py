...
def make_paths_for_test_data(source_dbs_path_prefix):
	return [
		(source_dbs_path_prefix.joinpath('Roaming/Mozilla/Firefox/Profiles/e0pj4lec.test_profile0', 'places.sqlite'),
		 ('firefox', 'test_profile0', 'places.sqlite'),
		 ('moz_places',),
		 ),
		
		(source_dbs_path_prefix.joinpath('Roaming/Mozilla/Firefox/Profiles/kceyj748.test_profile1', 'places.sqlite'),
		...
		(source_dbs_path_prefix.joinpath('Local/Google/Chrome/User Data/Profile 1', 'history'),
		 ('chrome', 'Profile 1', 'history'),
		 ('urls',),
		 ),
		
		(source_dbs_path_prefix.joinpath('Local/Vivaldi/User Data/Default', 'History'),
...
def get_data_yielder(db_paths_tables, fields_accessed):
	query = lambda table: f'SELECT * FROM {table}'
	for db_path_, (browser, profile, file), tables in db_paths_tables:
...
def make_benchmark_db(source_dbs_path_prefix, dst_db_path, fields_accessed, fields_added, fields_to_be_written):
	db_paths_tables = make_paths_for_test_data(source_dbs_path_prefix=source_dbs_path_prefix)
	row_yielder = get_data_yielder(db_paths_tables, fields_accessed)
...
def main():
	path_to_project = 'C:/Users/kshit/OneDrive/workspace'
	path_in_project_to_test_profiles = 'UnitedStatesOfBrowsers/tests/data/browser_profiles_for_testing/AppData'
	path_in_project_to_benchmark_db = 'UnitedStatesOfBrowsers/tests/data'
	benchmark_dbname = '~benchmark_db_for_usb_testing.sqlite'
	
	fields_accessed = ('url', 'title', 'last_visit_date', 'last_visit_time')
	fields_added = ('browser', 'profile', 'file', 'tablename')
	fields_to_be_written = ('url', 'title', 'last_visit', *fields_added)
	
	source_dbs_path_prefix = Path(path_to_project, path_in_project_to_test_profiles)
	dst_db_path = Path(path_to_project, path_in_project_to_benchmark_db, benchmark_dbname)
	
	if dst_db_path.exists():
		raise FileExistsError
	else:
		make_benchmark_db(source_dbs_path_prefix, dst_db_path, fields_accessed, fields_added, fields_to_be_written)
		dst_db_path.chmod(stat.S_IREAD)
