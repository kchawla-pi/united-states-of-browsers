...
def xtest_find_installed_browsers(tests_root):
    ...
    combined_db = DatabaseMergeOrchestrator(
        app_path=tests_root,
        db_name="test_combi_db",
    )
    assert combined_db.installed_browsers_data is None
    combined_db.installed_browsers_data = find_installed_browsers(
...
def test_make_records_yielder(tests_root):
    ...
    browser_counts = {
        browser_name_: browser_names.count(browser_name_)
    ...
    profile_counts = {
        profile_name_: profile_names.count(profile_name_)
...
def test_write_records():
    ...
    expected_records = [
        record for browser in mock_browsers_records for record in browser
    ]
    ...
    def _test_write_record_(tmp_dir):
        ...
        write_records(
            records_yielders=mock_records_generator,
            output_db=output_db,
            tablename=tablename,
...
def test_write_records_improper_table_name():
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with pytest.raises(ValueError) as excep:
            write_records(
                records_yielders=browser_yielder,
                output_db=output_db,
                tablename=tablename,
                primary_key_name="rec_num",
                fieldnames=["field1", "field2"],
        ...
        assert str(excep.value) == (
            "Table name cannot have spaces. " "You provided 'junk _table'"
        )
...
def _core_code_for_testing_db_merge(tmp_dir, browser_info):
    ...
    due to PermissionError with open file handles.
    """
    output_db = orchestrate_db_merge(
        app_path=tmp_dir,
        db_name="test_combi_db",
        browser_info=browser_info,
    )
...
@pytest.mark.skipif(
    ...
    reason="FTS5 unavailable; " "required for search table in test.",
)
...
@pytest.mark.skipif(
    check_fts5_installed(), reason="FTS5 avaliable, test inappropriate."
)
