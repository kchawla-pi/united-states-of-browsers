...
@pytest.mark.parametrize('test_case', [test_case for test_case in rdb_data.firefox_testdata['defaults']])
def test_firefox_defaults(test_case):
	actual_output = read_browser_db.firefox()
	assert test_case.expected == actual_output

@pytest.mark.parametrize('test_case', [test_case for test_case in rdb_data.firefox_testdata['values']])
def test_firefox_values(test_case):
	actual_output = read_browser_db.firefox(profiles=test_case.profiles)
	assert test_case.expected == actual_output

@pytest.mark.parametrize('test_case', [test_case for test_case in rdb_data.firefox_testdata['exceps']])
