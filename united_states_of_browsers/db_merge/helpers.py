...
def make_queries(
    ...
    queries = {
        "create": f"""CREATE TABLE IF NOT EXISTS {tablename} ({primary_key_name} integer PRIMARY KEY, {fieldnames_str[:]})"""
    }
    queries.update(
        {
            "insert": f"INSERT INTO {tablename} ({fieldnames_str}) VALUES ({query_placeholder[:-2]})"
        }
    )
    return queries
...
def query_sanitizer(query: str, allowed_chars: Union[str, Iterable] = "_") -> str:
    ...
    return "".join(
        [char for char in query if char.isalnum() or char in allowed_chars]
    )  # , '?', '(', ')', ','}])
...
def get_warnings_text(warnings_recorded: Iterable[Warning]) -> Mapping[Text, Warning]:
    ...
    """
    return {str(warning_.message): warning_.category for warning_ in warnings_recorded}
