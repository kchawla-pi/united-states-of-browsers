...
class Table(dict):
    ...
    def __init__(
        ...
        super().__init__(
            table=table, path=path, browser=browser, file=filename, profile=profile
        )
        self.table = table
        self.path = Path(path)
    ...
    def _check_spaces_in_table_name(self):
        ...
        if " " in self.table:
            raise ValueError(
                f"Table name cannot have spaces. You provided '{self.table}'"
            )

    def _create_db_copy(self):
        dst = Path(
            self.copies_subpath, "AppData", "Profile Copies", self.browser, self.profile
        ).expanduser()
    ...
    def _connect(self):
        ...
        try:
            files_pre_connection_attempt = set(
                entry for entry in self.path.parents[1].iterdir() if entry.is_file()
            )
        ...
        try:
            ...
            exceph.remove_new_empty_files(
                dirpath=self.path.parents[1],
                existing_files=files_pre_connection_attempt,
            )
    ...
    def _query_table(
        self, cursor: sqlite3.Connection.cursor, query: Text, raise_exceptions: bool
    ) -> Tuple[Generator, Exception]:
    ...
    def _yield_readable_timestamps(
        self, records_yielder: Generator, timestamp_field: [Mapping[Text, Text], None]
    ) -> Generator:
        ...
        """
        original_timestamp_fieldname, new_timestamp_fieldname = tuple(
            timestamp_field.items()
        )[0]
        for record in records_yielder:
            ...
            try:
                ...
                record_dict.update(
                    {new_timestamp_fieldname: str(human_readable).split(".")[0]}
                )
            yield record_dict

    def _check_if_timestamp_field_needed(
