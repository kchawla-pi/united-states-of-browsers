...
def write_records(
    records_yielders: List[Generator],
    output_db: PathInfo,
    tablename: Text,
    ...
    with sqlite3.connect(str(output_db)) as connection:
        ...
        records_yielder = (
            tuple(browser_record.values())
            for browser_record_yielder in records_yielders
...
def orchestrate_db_merge(
    app_path: PathInfo, db_name: Text, browser_info: BrowserData
) -> PathInfo:
    """
    ...
    write_records(
        records_yielders=browser_records_yielder,
        output_db=output_db,
        tablename="history",
    ...
    try:
        ...
        if str(excep) == "no such module: fts5":
            warnings.warn(
                "FTS5 extension for SQLIte not available/enabled. Search functionality unavailable."
            )
        else:
            raise excep
...
def merge_browsers_history(
    app_path: PathInfo, merged_db_name: Text
):  # pragma: no cover
    all_browsers_info = browser_data.prep_browsers_info()
    orchestrate_db_merge(
        app_path=app_path, db_name=merged_db_name, browser_info=all_browsers_info
    )
