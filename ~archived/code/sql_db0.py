...
def _profile_dir(profile_loc, profile_name='default'):
    try:
...
def _setup_paths(path=None, profile_name='default'):
    ...
    profile_dir_ = _profile_dir(profile_loc, profile_name)
    profile_path = os.path.join(profile_loc, *profile_dir_)
    return profile_path
...
def chrome():
    file_paths = _filepath(root="C:\\Users\\kshit\\AppData\\Local\\Google\\Chrome\\User Data\\Default",
